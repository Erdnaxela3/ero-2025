...
try:
    ...
    if  (not load):
        network = ox.graph_from_place("Outremont, Montreal, QC, Canada", network_type="drive")
        eulerized, path = eulerian_path(network.to_undirected())
        pickle.dump(path, open("../../Outremont-drone.p", "wb"))
        pickle.dump(eulerized, open("../../Outremont-eulerized.p", "wb"))
    else :
        eulerized= pickle_load("../../Outremont-eulerized.p")
        path = pickle_load("../../Outremont-drone.p")
    
    graph = [opti(time, budget, eulerized, path, n) for n in range(1,vehicle + 1)]
    display_graph(graph, str(budget), str(time), "Outremont", vehicle + 1)
