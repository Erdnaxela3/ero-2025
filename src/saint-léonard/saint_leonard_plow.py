...
try:
    ...
    if  (not load):
        network = ox.graph_from_place("Saint-Léonard, Montreal, QC, Canada", network_type="drive")
        eulerized, path = eulerian_path(network.to_undirected())
        pickle.dump(path, open("../../Saint-Léonard-drone.p", "wb"))
        pickle.dump(eulerized, open("../../Saint-Léonard-eulerized.p", "wb"))
    else :
        eulerized= pickle_load("../../Saint-Léonard-eulerized.p")
