...
try:
    ...
    if  (not load):
        network = ox.graph_from_place("Verdun, Montreal, QC, Canada", network_type="drive")
        eulerized, path = eulerian_path(network.to_undirected())
        pickle.dump(path, open("../../Verdun-drone.p", "wb"))
        pickle.dump(eulerized, open("../../Verdun-eulerized.p", "wb"))
    else :
        eulerized= pickle_load("../../Verdun-eulerized.p")
        path = pickle_load("../../Verdun-drone.p")
    
    graph = [opti(time, budget, eulerized, path, n) for n in range(1,vehicle + 1)]
    display_graph(graph, str(budget), str(time), "Verdun", vehicle + 1)
